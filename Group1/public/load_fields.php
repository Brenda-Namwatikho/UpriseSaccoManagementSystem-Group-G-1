...
extract($_POST);
if(isset($id)){
	$qry = $conn->query("SELECT * FROM payment where id=".$memberNumber);
	foreach($qry->fetch_array() as $k => $val){
		$$k = $val;
	}
}
$loan = $conn->query("SELECT l.*,b.name, b.contact, b.address from acceptedloan l inner join user b on b.id = l.memberNumber where l.id = ".$loanNumber);
...
$payments = $conn->query("SELECT * from payment where loan_id =".$loan_id);
